...
public class QuesContentProvider extends ContentProvider
{
	private QuesSqliteHelper dbOpenHelper;
	...
	{
		dbOpenHelper = new QuesSqliteHelper(this.getContext());
		return true;
	}
	...
	public int delete_ques(Uri uri, String selection, String[] selectionArgs)
	{
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		int count = 0;
		switch (sMatcher.match(uri))
		...
		//uri="content://com.chen.provider.userprovider/person"
		case USERS:
			count = db.delete("q_ques", selection, selectionArgs);
			break;
		default:
			throw new IllegalArgumentException("Unknow Uri:" + uri);
		}
...
//			pid = db.insert("u_user", "upassword", values);
	...
	public Uri insert_ques(Uri uri, ContentValues values)
	{
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		long pid = 0;
		switch (sMatcher.match(uri))
		{
		case USERS:
			pid = db.insert("q_ques", "question_show", values);
			return ContentUris.withAppendedId(uri, pid);
		default:
			throw new IllegalArgumentException("Unknow Uri:" + uri);
		}
	...
	public Cursor query_ques(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder)
	{
		SQLiteDatabase db = dbOpenHelper.getReadableDatabase();
		switch (sMatcher.match(uri))
		{
		case USERS:
			return db.query("q_ques", projection, selection, selectionArgs, null, null, sortOrder);
		default:
			throw new IllegalArgumentException("Unknow Uri:" + uri);
		}
	}
	...
	public int update_ques(Uri uri, ContentValues values, String selection, String[] selectionArgs)
	{
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		int count = 0;
		switch (sMatcher.match(uri))
		...
		case USERS:
			//content://com.chen.provider.userprovider/user
			count = db.update("q_ques", values, selection, selectionArgs);
			break;
		default:
			throw new IllegalArgumentException("Unknow Uri:" + uri);
		}
